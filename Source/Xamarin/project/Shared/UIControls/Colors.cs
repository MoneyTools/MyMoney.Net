...
namespace XMoney
{
    ...
    {
        public static Color GetCurrencyColor(decimal value)
        {
            return value < 0 ? Color.DarkRed : Color.DarkGreen;
        }

        private static readonly Random rnd = new();
        private static readonly List<Color> listOfGoodColors = GenerateColors();
        ...
        public static SKColor Darker(SKColor color, float percentage)
        {
            byte R = (byte)(color.Red * percentage);
            byte G = (byte)(color.Green * percentage);
            byte B = (byte)(color.Blue * percentage);
        ...
        private static List<Color> GenerateColors()
        {
            ...
            "#3D7397", "#CAE8CE", "#D60034", "#AA6746", "#9E5585", "#BA6200","#1CE6FF", "#FF34FF",
            "#FF4A46", "#008941", "#006FA6", "#A30059"};

            var list = new List<Color>();

            foreach (string s in indexcolors)
            {
                SKColor sc = SKColor.Parse(s);
                list.Add(Color.FromRgb(sc.Red, sc.Green, sc.Blue));
            }

            //var sortedColors = list.OrderBy(c => c.Hue)
            //.ThenBy(c => c.Saturation)
            //.ThenBy(c => c.Luminosity);

            //return sortedColors.ToList();
            return list;
        }

        private static int GetIndexInRange(int index, int max)
        {
            if (index < max)
            {
                return index;
            ...
            index -= max;
            return GetIndexInRange(index, max);
        }

        public static Color GetColor(int index)
        {
            index = GetIndexInRange(index, listOfGoodColors.Count() - 1);
            return listOfGoodColors[index];
        }
        public static string GenerateColorFromWords(string input, bool moreRed, bool moreGreen)
        {
            if (moreRed)
            {
                return "#AA" + GetChecksum(input, 4);
            }

            if (moreGreen)
            {
                string tmp = GetChecksum(input, 4);
                return "#" + tmp.Substring(0, 2) + "AA" + tmp.Substring(2, 2);
            }

            return "#" + GetChecksum(input, 6);
        }

        public static string GetChecksum(string s, int len)
        {
            int checksum = 0; //16777215; // FFFFFF
            ...
            return checksum.ToString("X" + len.ToString());
        }

        public static List<Color> ListOfGoodColors()
        {
            int interval = 40;

            var colors = new List<Color>();
            for (int red = 10; red < 200; red += interval)
            {
                for (int green = 10; green < 200; green += interval)
                {
                    for (int blue = 10; blue < 200; blue += interval)
                    {
                        if (red > 150 | blue > 150 | green > 150) //to make sure color is not too dark
                        {
                            colors.Add(Color.FromRgb((byte)red, (byte)green, (byte)blue));
                        }
                    }
                }
            }

            IOrderedEnumerable<Color> sortedColors = colors.OrderBy(c => c.Hue)
                                     .ThenBy(c => c.Saturation)
                                     .ThenBy(c => c.Luminosity);
    ...
    {
        private readonly Color RowColorEven = Color.FromHex("F5F9F8");
        private readonly Color RowColorOdd = Color.FromHex("EEEEFF");

        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            Color rowcolor = RowColorOdd;
            if (value == null || parameter == null)
            {
                return rowcolor;
            }

            int index = ((ListView)parameter).ItemsSource.Cast<object>().ToList().IndexOf(value);
            if (index % 2 == 0)
            {
